...
const usersList = ({users, partyEnded}) => {
    ...
    return (
        <div className={`border border-primary rounded-md bg-secondary select-none ${partyEnded ? 'w-1/2 mx-auto' : 'w-fit cursor-pointer'}`} onClick={() => {
            if(!partyEnded) {
            ...
            <div className={"flex flex-row gap-x-4 items-center ease-in-out duration-300 hover:bg-primary hover:text-white px-5 pt-3 pb-4"}>
                <p>Liste des utilisateurs {partyEnded && '- Score final'} </p>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5"
                     stroke="currentColor" className={`w-6 h-6 duration-300 ${expandUserList ? 'rotate-180' : 'rotate-0'}  ${partyEnded ? 'hidden' : ''}`}>
