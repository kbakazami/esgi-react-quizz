...
export default function RoomParty({socket, users, roomInformations, questionInformations, roomQuestion}) {
    ...
    return (
        <div>
            ...
            <div className={`flex ${roomInformations.partyEnded ? 'flex-col gap-y-10 justify-center items-center' : 'flex-row justify-between items-start'}`}>
                ...
                <UsersList users={users} partyEnded={roomInformations.partyEnded}/>
            ...
            </div>
            {
                roomInformations && !roomInformations.partyEnded && (
                    <>
                        <Questions roomInformations={roomInformations} roomQuestion={roomQuestion} socket={socket} resetSubmittedAnswer={false}/>
